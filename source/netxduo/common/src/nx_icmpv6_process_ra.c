        ...
        if (option_ptr -> nx_icmpv6_option_type == ICMPV6_OPTION_TYPE_PREFIX_INFO)
        {
            ...
            if (prefix_ptr -> nx_icmpv6_option_prefix_flag & 0x80)
            {
                ...
                else
                {
                    ...
                    /* If router advertisement prefix callback is set, invoke the callback function. */
                    if (ip_ptr -> nx_icmpv6_ra_prefix_callback)
                    {
                        ip_ptr -> nx_icmpv6_ra_prefix_callback(ip_ptr, prefix_ptr ->nx_icmpv6_option_prefix,
                                                                (ULONG)prefix_length);
                    }
